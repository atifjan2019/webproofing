<div class="tabs-nav">
    <a href="{{ route('sites.show', $site) }}" class="tab-link {{ request()->routeIs('sites.show') ? 'active' : '' }}">
        <svg class="icon-md" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
        </svg>
        Overview
    </a>

    <a href="{{ route('sites.screenshots', $site) }}"
        class="tab-link {{ request()->routeIs('sites.screenshots') ? 'active' : '' }}">
        <svg class="icon-md" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
        </svg>
        Screenshots
    </a>

    @if(Auth::user()->googleAccount)
        <a href="{{ route('sites.analytics', $site) }}"
            class="tab-link {{ request()->routeIs('sites.analytics') ? 'active' : '' }}">
            <svg class="icon-md" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
            </svg>
            Analytics
        </a>
    @endif

    <!-- PageSpeed Tab -->
    <a href="{{ route('sites.pagespeed', $site) }}"
        class="tab-link {{ request()->routeIs('sites.pagespeed') ? 'active' : '' }}">
        <svg class="icon-md" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
        </svg>
        Speed Test
    </a>

    <a href="{{ route('sites.google', $site) }}"
        class="tab-link {{ request()->routeIs('sites.google*') ? 'active' : '' }}">
        <svg class="icon-md" viewBox="0 0 24 24">
            <path fill="currentColor"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
            <path fill="currentColor"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
        </svg>
        Google Services
    </a>
</div>